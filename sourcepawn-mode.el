;;; sourcepawn-mode.el --- SourcePawn major mode
...
(defvar sourcepawn-mode-hook
  nil
  "Hook called by `sourcepawn-mode'.")

;; breaking down the variable name match regexp into parts
(defvar sourcepawn-mode-font-lock-regexp-variable-names
  "\\(?:\\(?:\\sw\\)+:\\)?\\(\\(?:\\sw\\)+\\)\\(?:[ \t]*\\[[^,;\n]*\\]\\)?\\(?:[ \t]*=[ \t]*\\(?:\\sw\\|\\s_\\|[ \t]\\)+\\)?"
  "Match the list part of SourcePawn variable declarations.
The list part consists of strings like 'String:test[256] = \"test\"'.
The variable name must be in group 1.")

(defvar sourcepawn-mode-font-lock-regexp-variable-names-prefix
  "\\(?:new\\|decl\\)[ \t]+"
  "Match the part of a variable declaration before the variable names list.
Must have no numbered groups.")

(defvar sourcepawn-mode-font-lock-regexp-variable-names-seperator
  "[ \t]*,[ \t]*"
  "Match the seperator between a variable declaration list element.
Must have no numbered groups.")

;; helper to tell us when our last match succeeded
(defvar sourcepawn-mode-font-lock-flag-inside-variable-declaration
  nil
  "When t, indicates the matcher is inside a variable declaration.
This is used by `sourcepawn-mode-font-lock-matcher-variable-names'.")
...
The argument LIMIT is a buffer position that bounds the match."
...
(defun sourcepawn-mode-single-line-block-p ()
  "Find the indentation of a single-expression block, or nil.
